...
public class CorrespondenceDocumentLookupSuggester implements  Suggester {
    ...
    public List<Suggestion> suggest(String userInput, SuggestionContext context) throws SuggestionException {
        ...
        try {
            ...
            if (providerName.equals("VIP_CORRESPONDENCE_SUGGESTION")){
                org.nuxeo.ecm.automation.core.util.Properties namedParameters = new Properties();
                List<String> visitedDocs = new ArrayList<String>();

                namedParameters.put("signUser", context.principal.getName());
                namedParameters.put("redirectUser", context.principal.getName());
                DocumentModel searchDocumentModel = PageProviderHelper.getSearchDocumentModel(context.session,
                        "VIP_CORRESPONDENCE_SUGGESTION", namedParameters);
                ...
                pp = (PageProvider<DocumentModel>) ppService.getPageProvider(providerName,
                        null, null, null, props, resultsHighlights, null, objects);
            }
            for (DocumentModel doc : pp.getCurrentPage()) {
                suggestions.add(CorrespondenceDocumentLookupSuggestion.fromDocumentModel(doc));
            }
            return suggestions;
