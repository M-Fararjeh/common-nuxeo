...
import org.nuxeo.ecm.automation.server.jaxrs.batch.*;
...
public class CustomBatchUploadObject extends BatchUploadObject {
    public static final String EXTENSIONS = "org.nuxeo.upload.custom.extensions";
    public static final String EXTENSIONS_MODE_PROP = "org.nuxeo.upload.custom.extensions.mode";
    public static final String UPLOAD_MAX_FILE_SIZE_PROP = "org.nuxeo.upload.custom.max.size";
    public static final String EXTENSIONS_MODE_EXCLUDE = "exclude";
    public static final String EXTENSIONS_MODE_INCLUDE = "include";
    ...
    protected Response uploadNoTransaction(@Context HttpServletRequest request,
        ...
        String extensionsMode = Framework.getProperty(EXTENSIONS_MODE_PROP, EXTENSIONS_MODE_INCLUDE);
        boolean isIncludeExtensions = extensionsMode.trim().compareToIgnoreCase(EXTENSIONS_MODE_INCLUDE) == 0;
        long maxFileSize = Long.parseLong(Framework.getProperty(UPLOAD_MAX_FILE_SIZE_PROP, "-1"));
        Set<String> allowedExt = null;
        HashMap<Object, Object> allowedExtHM = new HashMap<>();
        if(!StringUtils.isEmpty(extensions.trim())) {
            allowedExt = Stream.of(extensions.trim().split("\\s*,\\s*"))
                    .collect(Collectors.toSet());

            allowedExt.forEach(ext -> {
                allowedExtHM.put(ext, Boolean.TRUE);
            });
        }
        ...
        if(!StringUtils.isEmpty(extensions.trim())) {
            int extensionStartIndx = fileName.lastIndexOf('.');
            if (extensionStartIndx > -1 && extensionStartIndx < fileName.length() - 1) {
                String extension = fileName.substring(extensionStartIndx + 1);
                // skip extensions

                if (allowedExtHM != null && !allowedExtHM.isEmpty() &&
                        ((isIncludeExtensions && !allowedExtHM.containsKey(extension.toLowerCase())) || (!isIncludeExtensions && allowedExtHM.containsKey(extension.toLowerCase())) )) {
//                    throw new NuxeoException("file type is not supported");
                    return buildTextResponse(Status.BAD_REQUEST, "file type is not supported");
                }
            }
        }
            ...
            try {
                ...
                try {
                    if (maxFileSize >= 0 && fileSize > maxFileSize) {
                        long maxFileSizeMB = maxFileSize / (1024L * 1024L);
//                        throw new IllegalParameterException(
//                                String.format("File size must be less than or equal to %d MB.", maxFileSizeMB));
                        return buildTextResponse(Status.BAD_REQUEST, String.format("File size must be less than or equal to %d MB.", maxFileSizeMB));
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
        ...
        if (isMultipart) {
            ...
            try {
                if (maxFileSize >= 0 && (uploadedSize > maxFileSize || fileSize > maxFileSize)) {
                    long maxFileSizeMB = maxFileSize / (1024L * 1024L);
                    return buildTextResponse(Status.BAD_REQUEST, String.format("File size must be less than or equal to %d MB.", maxFileSizeMB));
                }
            } catch (Exception e) {
    ...
    public Response complete(@PathParam(REQUEST_BATCH_ID) String batchId,
                                    @PathParam(REQUEST_FILE_IDX) String fileIdx, String body) throws IOException {
        /* Prepare file extensions properties*/
        String extensions = Framework.getProperty(EXTENSIONS, "");
        String extensionsMode = Framework.getProperty(EXTENSIONS_MODE_PROP, EXTENSIONS_MODE_INCLUDE);
        boolean isIncludeExtensions = extensionsMode.trim().compareToIgnoreCase(EXTENSIONS_MODE_INCLUDE) == 0;
        long maxFileSize = Long.parseLong(Framework.getProperty(UPLOAD_MAX_FILE_SIZE_PROP, "-1"));
        ...
        try {
            if(!StringUtils.isEmpty(extensions.trim()) && filename != null) {
                int extensionStartIndx = filename.lastIndexOf('.');
                if (extensionStartIndx > -1 && extensionStartIndx < filename.length() - 1) {
                    String extension = filename.substring(extensionStartIndx + 1);
                    // skip extensions

                    if (!allowedExtHM.isEmpty() &&
                            ((isIncludeExtensions && !allowedExtHM.containsKey(extension.toLowerCase())) || (!isIncludeExtensions && allowedExtHM.containsKey(extension.toLowerCase())) )) {
//                    throw new NuxeoException("file type is not supported");
                        bm.clean(batchId);
                        return buildTextResponse(Status.BAD_REQUEST, "file type is not supported");
